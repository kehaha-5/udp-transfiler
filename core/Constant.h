...
const int MAX_SEND_PACKETS =
    (std::ceil(MAX_DOWNLOAD_SPPED / MAX_FILE_DATA_SIZE) == 0
         ? 1
         : std::ceil(MAX_DOWNLOAD_SPPED / MAX_FILE_DATA_SIZE));  //在下载时最多每秒发送多少个包
                                                                 // MAX_SEND_PACKETS_NUM_PEER_SECONDE*MAX_FILE_DATA_SIZE等于大概下载速度
                                                                 // 还要考虑下载线程数量 MAX_ACK_SET_SIZE数量
const int MAX_ACK_SET_SIZE =
    500;  //可以处理最大的重发次数，在重发次数上升时，线程池的线程主要都去处理重发数据，导致没有人处理接受数据，导致更多的数据超时重发，当重发次数大于此数时，无法再发送新包

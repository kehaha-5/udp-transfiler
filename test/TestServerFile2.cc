...
class TestServerFile2 : public ::testing::Test {
   protected:
    void SetUp() {
...
TEST_F(TestServerFile2, functionalTestFile) {
    File file(_textFile);
...
TEST_F(TestServerFile2, RandomReadBinaryFile) {
    // 测试二进制文件正常读取
    File file(_binFile);  // 请根据你的文件类构造函数进行修改
    for (int i = 0; i < 26; i++) {
        for (int j = (i == 0) ? 1 : i; j < 26; j++) {
            fileData testData;
            auto res = file.getPosContext2(i, j, testData);
            EXPECT_TRUE(res);
            if (!res) {
...
TEST_F(TestServerFile2, multiplyThreadReadTest) {

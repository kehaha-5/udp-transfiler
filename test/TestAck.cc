...
TEST(AckTest, functionalTest) {
    ...
    for (int i = 0; i < loopNum; i++) {
        unqiueAck.insert(ack::AckRandom::getAck());
    }
    EXPECT_EQ(unqiueAck.size(), loopNum);
    EXPECT_EQ(unqiueAck.size(), ack::AckRandom::getAckMapSzie());

    int j = 1;
    for (auto ack : unqiueAck) {
        ASSERT_EQ(1, ack::AckRandom::releasesAck(ack));
        if (j == delItemNum) {
            EXPECT_EQ(loopNum - delItemNum, ack::AckRandom::getAckMapSzie());
            delItemNum += 50;
        }
        j++;
    }
    ...
    ASSERT_EQ(ack::AckRandom::getAckMapSzie(), 0);
}
...
TEST(AckTest, multiplyThreadTest) {
    ...
    ASSERT_EQ(ack::AckRandom::getAckMapSzie(), threadNum * threadLoopNum);
}
